---
title: "Beyond Infrastructure: Understanding the Causal Impact of Bike Lane Installation on Urban Cycling Adoption"
subtitle: "A Difference-in-Differences Analysis of Toronto's Cycling Network, 2017-2023"
author: 
  - Steven Li
...
\newpage
# Introduction
Cycling infrastructure plays a crucial role in urban transportation, yet quantifying its impact on cycling adoption remains challenging. While protected bike lanes can reduce cycling injuries by up to 89% [@harris2020lane] and increase ridership by up to 171% on specific routes [@nacto2016], less is known about how different types of infrastructure improvements affect bike share system utilization. This knowledge gap is particularly relevant for Toronto, where rapid expansion of cycling infrastructure has coincided with dramatic growth in the bike share network from 2,854 to over 6,850 stations between 2016 and 2020 [@toronto_bike_share_expansion]. Understanding this relationship is critical as cities increasingly rely on bike share systems to complement public transit and provide sustainable transportation options.

We estimate the causal effect of cycling infrastructure changes on Bike Share Toronto ridership between 2019-2021, examining how protected lanes, on-road lanes, and shared roadways influence system usage. Using a difference-in-differences design, we analyze both new installations and infrastructure upgrades while controlling for broader trends in cycling adoption.

Our analysis reveals substantial increases in bike share ridership following infrastructure improvements. Post-implementation effects reach 133.1 additional monthly rides per bikeway (p < 0.01), translating to approximately 300,000 additional annual rides across all 186 treated segments - particularly notable as these improvements represent only 16% of the 1,191 bikeways in our study. Protected lanes demonstrate the strongest impact, generating 381.9 additional monthly rides (p < 0.001) - more than double the effect of on-road lanes and triple that of shared roadways. These gains occurred despite treated bikeways having higher initial ridership at 209.03 monthly rides, compared to control bikeways 85.59 monthly rides pre-treatment. This suggests infrastructure improvements can substantially increase cycling activity even on popular routes.

These findings carry important implications for urban transportation policy. Toronto's planned addition of 100 kilometers of cycling infrastructure between 2025-2027 [@cyclingNetworkPlan] presents an opportunity to enhance cycling adoption through strategic implementation. Our results challenge conventional assumptions about prioritizing underserved areas, as improvements to high-traffic routes generated significant additional ridership, highlighting the complex trade-offs in infrastructure resource allocation.
...
# Data {#sec-data}
## Overview
This study draws on three datasets from the City of Toronto's Open Data Portal [@torontoOpenData]: the Cycling Network (Bikeways) data [@bikeWayData], and both Bike Share Ridership and Stations data [@bikeShareData]. Together, these datasets provide a comprehensive view of Toronto's cycling landscape, capturing infrastructure development from 2001-2023 and ridership patterns from 2017-2023. All data cleaning and analysis was conducted using R [@RCoreTeam2023], with key packages including `tidyverse` [@tidyverse], `dplyr` [@dplyr], and `sf` [@sf] for spatial analysis.
...
A crucial methodological choice was the use of relative timing rather than calendar years. Due to bikeways having different upgrade or installation years, we align their treatment event year using a relative period. For treated bikeways, this creates a 60-month window: 24 months pre-treatment, 12 months during the treatment year, and 24 months post-treatment. Control bikeways receive randomly assigned pseudo-treatment years (2019, 2020, or 2021) to enable parallel analysis. This approach allows us facilitate direct comparison between treatment and control groups while accounting for the temporal distribution of infrastructure changes.

@tbl-sample-data in Appendix @sec-additional-figures provides a sample of our processed dataset.

## Outcome Variables
Our primary outcome variable is seasonally-adjusted monthly ridership for each bikeway (`monthly_rides_adj`). @fig-avg-monthly-ridership reveals several important patterns in ridership behavior. The raw ridership (black line) displays pronounced seasonal fluctuations, with consistent peaks during summer months and troughs in winter, reflecting Toronto's strong cycling seasonality. After applying seasonal adjustments (blue line), these cyclical patterns are smoothed while preserving the underlying growth trends. Both treatment and control groups show a general upward trend in ridership over the observation period, suggesting broader growth in cycling activity across Toronto's network.
...
#| fig-cap: "Average ridership by relative months to treatment"
#| echo: false
#| warning: false
#| message: false
...
ggplot(ridership_by_month, aes(x = relative_month, y = rides, color = metric)) +
  ...
  scale_color_manual(
    ...
    labels = c("Raw Rides", "Seasonally Adjusted")
  ) +
  theme_minimal() +
...
Appendix @sec-additional-figures provide a further breakdown on types of bikeways installed and upgraded throughout the years. @fig-bikeway_type_constructed_graph in the appendix shows consistent bikeway installations every year after 2010, with protected lanes most commonly installed. @fig-bikeway_type_upgraded_graph presents yearly bikeway upgrades, showing an increase during our treatment period (2019-2021), especially with protected bike lanes.

```{r}
#| label: fig-treatment-control-count
#| fig-cap: "Total Bikeway Count in Control Vs. Treatment Group"
#| echo: false
#| warning: false
#| message: false
...
#| fig-cap: "Infrastrucutre Type Distribution between Control and Treatment Group"
#| echo: false
#| warning: false
#| message: false
...
#| fig-cap: "Number of New-Installations Vs Upgrades in Treatment Group"
#| echo: false
#| warning: false
#| message: false
...
Our analysis is ran in R [@RCoreTeam2023] using the `fixest` package [@fixest], which provides estimation methods for fixed effects models. This package is particularly suitable for our analysis as it handles multiple fixed effects efficiently and provides robust standard errors clustered at the bikeway level.
...
We extend our analysis to examine treatment effect heterogeneity across different types of infrastructure changes. This investigation is crucial for understanding which interventions are most effective at increasing ridership. We estimate two separate models:
...
modelsummary(
  list("Placebo Test - Random Assignment" = placebo_random_did),
  stars = TRUE,
  gof_map = c("nobs", "r.squared", "adj.r.squared")
)
...
modelsummary(
  list("Heterogeneous Effects - Bikeway Type" = did_hetero_lane),
  stars = TRUE,
  gof_map = c("nobs", "r.squared", "adj.r.squared"),
  coef_map = c(
  "period::treatment:treatment_infra::Control" = "Treatment x Control",
  "period::treatment:treatment_infra::Treated-OnRoad" = "Treatment x Treated-OnRoad",
  "period::treatment:treatment_infra::Treated-Protected" = "Treatment x Treated-Protected",
  "period::treatment:treatment_infra::Treated-Shared" = "Treatment x Treated-Shared",
  "period::post:treatment_infra::Control" = "Post x Control",
  "period::post:treatment_infra::Treated-OnRoad" = "Post x Treated-OnRoad",
  "period::post:treatment_infra::Treated-Protected" = "Post x Treated-Protected",
  "period::post:treatment_infra::Treated-Shared" = "Post x Treated-Shared"
  ),
  output = "latex"
)
```
...
modelsummary(
  list("Heterogeneous Effects - Treatment Type" = did_hetero_treat_type),
  stars = TRUE,
  gof_map = c("nobs", "r.squared", "adj.r.squared"),
  coef_map = c(
    "period::treatment:sub_treatment_type::Upgraded" = "Treatment x Upgraded",
    "period::treatment:sub_treatment_type::Newly-Installed" = "Treatment x Newly Installed",
    "period::post:sub_treatment_type::Upgraded" = "Post x Upgraded",
    "period::post:sub_treatment_type::Newly-Installed" = "Post x Newly Installed"
  ),
  output = "latex"
)
```

These heterogeneity analyses reveal important nuances in the effectiveness of different infrastructure strategies. The particularly strong effect of protected lanes suggests that safety and separation from vehicle traffic may be key factors in encouraging Bike Share usage. Additionally, the stronger effects of upgrades compared to new installations may indicate the importance of improving infrastructure in areas where cycling activity already exists.

# Discussion {#sec-discussion}
Our difference-in-differences analysis of Toronto's cycling infrastructure changes between 2019-2021 reveals nuanced insights about how different types of cycling infrastructure improvements influence Bike Share utilization. The results demonstrate that while infrastructure investments broadly increase cycling adoption, the magnitude and timing of these effects vary substantially by implementation approach and infrastructure type.
...
- **Neighborhood-level analysis**: Future research could examine how infrastructure effects vary across Toronto's diverse neighborhoods. Such analysis would be particularly valuable as Toronto expands Bike Share into new areas, helping identify where different types of infrastructure improvements might have the greatest impact. This geographic heterogeneity analysis could also inform equity considerations in infrastructure deployment, ensuring benefits are distributed across the city's diverse communities.
...
#| tbl-cap: "Sample of processed dataset for DiD Model"
#| echo: false
#| warning: false
#| message: false
